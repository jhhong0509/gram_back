...
- 여러줄 주석
    ...
    - /** 주석 내애애용 */도큐먼트 주석을 사용할 있다고 한다. 도큐먼트 주석은 함수에 대한 설명 같은 것을 넣는 것이다.
# 변수
- 변수란
    - 변수란 데이터를 저장하는 메모리 공간을 말한다.
    - 변수는 변수를 특정하는 이름을 가지고, 하나의 변수는 하나의 값만 저장할 수 있다.(배열은 변수를 여러개 만든 것 이므로, 하나의 변수에는 하나의 값만 들어간다.)
- 변수의 선언과 사용
    - 변수의 선언
...
- 특징
    - 여러줄을 사용할 수 있다.
    - return을 할 수 있다.
    - 실행문에 return 할 것 만 있으면 그냥 식만 쓰면 된다.
- 함수적 인터페이스란
    - 추상메소드가 하나인 인터페이스
- 함수적 인터페이스 사용
    - 인터페이스명 변수 = 람다;
    - 인터페이스의 구현객체를 람다식이 만들어준다.
    - 추상메소드가 2개 이상이면 불가능
    - 추상메소드가 2개이상인지 확인하는 어노테이션
    ```java
    @FuntionalInterface
    ```
    - 만약 2개 이상이면 컴파일 에러
# 클래스 멤버와 로컬 변수 사용
- 람다식 특징
    - 람다식은 바깥쪽의 클래스 멤버를 제약없이 사용 가능
- 로컬변수 특징
    - 로컬변수는 주로 메소드 안에 작성됨
    - 객객체나 람다식이 만드는 객체는 소멸되지 않지만 메소드와 메소드의 매개변수, 로컬변수는 소멸된다.
# 함수적 인터페이스
- 함수적 인터페이스란?
    - 위에서 말했듯이 한개의 추상 메소드만 가지는 인터페이스다.
    - 람다식으로 표현할 수 있다.
- Consumer 함수적 인터페이스
###### 하하 또다른곳에서  막혔다!!@!#!@#!@#!@#!@#
# 메소드 참조
- 메소드 참조란
    - 메소드를 단순 호출하거나 매개값만 전달하는 람다식을 생략하는 방법
    ```java
...
- Map
    ...
    - 컬렉션 루핑
        - 컬렉션 루핑이란
            - Map 컬렉션의 객체를 하나씩 얻어서 반복문을 돌리는 방법
            - Iterator 을 사용한다.
                - Iterator이란?
                    - Iterator이란 반복자이다.
                    - 메소드
                        - hasNext()
                            - 읽을 요소가 있는지 검사
                        - next()
                            - hasNext와 비슷하지만 다음 요소를 반환한다.
                        - remove()
                            - next로 얻은 요소 삭제
                    - Iterator<타입> 변수 이름 = keySet.iterator()
                    처럼 선언한 후에 Iterator.hasNext()로 반복문을 돌리면 된다.